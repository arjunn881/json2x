 ...
 * build-tool-pseo.js
 * ─────────────────────────────────────────────────────────────
 * Programmatic SEO Engine — Per-Tool Keyword Pages
 *
 * Strategy: For each tool, generate multiple keyword-targeted pages
 * covering the 4 primary search intents:
 *   1. "Tool Name Online" → informational (how it works)
 *   2. "Tool Name Free" → commercial (feature comparison)
 *   3. "Tool Name Alternative" → comparison (vs competitors)
 *   4. "Tool Name [Use Case]" → use-case specific (developer persona)
 *
 * Output directory: /kb/tools/
 * Hub page: /kb/tools/index.html
 *
 * Each generated page:
 *   - Unique <title>, <meta description>, canonical URL
 *   - H1, H2, H3 hierarchy targeting the keyword
 *   - FAQ schema (FAQPage JSON-LD)
 *   - TechArticle + BreadcrumbList schema
 *   - Internal links to the actual tool page + related tools
 *   - Full CTA with direct link to the tool
 *   - 400–600 words of unique, high-quality body content
 */
...
const OUT_DIR  = path.join(WORKSPACE_ROOT, 'kb', 'tools');
...
✅ Format     → json2x.com/tools/json-formatter.html
✅ Validate   → json2x.com/tools/json-validator.html
✅ Minify     → json2x.com/tools/json-minifier.html
✅ Diff       → json2x.com/tools/json-diff.html
✅ Tree View  → json2x.com/tools/json-tree-viewer.html
✅ → CSV      → json2x.com/tools/json-to-csv.html
✅ → YAML     → json2x.com/tools/json-to-yaml.html
✅ → XML      → json2x.com/tools/json-to-xml.html
✅ → SQL      → json2x.com/tools/json-to-sql.html
✅ → TypeScript → json2x.com/tools/typescript-generator.html
✅ → Zod      → json2x.com/tools/json-to-zod.html
✅ → Prisma   → json2x.com/tools/json-to-prisma.html
✅ → Drizzle  → json2x.com/tools/json-to-drizzle.html
✅ → GraphQL  → json2x.com/tools/json-to-graphql.html`,
    faqs: [
...
// Test it yourself:
// 1. Load json2x.com/tools/json-formatter.html
...
// ── HTML Template ──────────────────────────────────────────────
function renderToolPseoHtml(page, allPages) {
  const canonicalUrl = `${BASE_URL}/kb/tools/${page.slug}.html`;
  const toolUrl = `${BASE_URL}/tools/${page.toolSlug}.html`;

  const relatedLinksHtml = (page.relatedTools || [])
    .slice(0, 5)
    .map(slug => {
      const found = allPages.find(p => p.toolSlug === slug);
      const label = found ? found.h1.split(':')[0] : slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      return `<li><a href="${BASE_URL}/tools/${slug}.html" style="color:var(--accent);text-decoration:none;">${label} →</a></li>`;
    })
    .join('\n');

  const otherArticlesHtml = allPages
    .filter(p => p.slug !== page.slug && p.toolSlug === page.toolSlug)
    .slice(0, 3)
    .map(p => `<li><a href="${BASE_URL}/kb/tools/${p.slug}.html" style="color:var(--accent);text-decoration:none;">${p.h1}</a></li>`)
    .join('\n');

  const faqCardsHtml = page.faqs.map(f => `
    <div class="faq-card">
      <h3 class="faq-card__q">${f.q}</h3>
      <p class="faq-card__a">${f.a}</p>
    </div>`).join('\n');

  const faqSchema = page.faqs.map(f => ({
    '@type': 'Question',
    'name': f.q,
    ...
    '@graph': [
      {
        ...
        'url': canonicalUrl,
        'about': { '@type': 'SoftwareApplication', 'name': page.h1.split(':')[0], 'url': toolUrl },
        'publisher': { '@type': 'Organization', 'name': 'JSON2X', 'url': BASE_URL }
      },
      {
        '@type': 'BreadcrumbList',
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 3, 'name': page.category, 'item': `${BASE_URL}/kb/tools/index.html` },
          { '@type': 'ListItem', 'position': 4, 'name': page.h1.split(':')[0], 'item': canonicalUrl }
        ]
      },
      {
        '@type': 'FAQPage',
...
<head>
  ...
  <style>
    .prose h2 { font-size:var(--text-xl); font-weight:var(--font-bold); margin:var(--space-8) 0 var(--space-3); }
    .prose h3 { font-size:var(--text-base); font-weight:var(--font-semibold); margin:var(--space-5) 0 var(--space-2); color:var(--text-secondary); }
    .prose p  { font-size:var(--text-base); color:var(--text-secondary); line-height:var(--leading-relaxed); margin-bottom:var(--space-4); }
    .prose ul, .prose ol { padding-left:var(--space-6); margin-bottom:var(--space-4); }
    .prose li { font-size:var(--text-base); color:var(--text-secondary); line-height:var(--leading-relaxed); margin-bottom:var(--space-2); }
    .prose code { font-family:var(--font-mono); font-size:0.85em; background:var(--bg-raised); border:1px solid var(--bg-border); border-radius:var(--radius-sm); padding:1px 5px; color:var(--accent); }
    .prose a { color:var(--accent); }
    .code-block { background:var(--bg-base); border:1px solid var(--bg-border); border-radius:var(--radius-lg); padding:var(--space-5); font-family:var(--font-mono); font-size:var(--text-xs); line-height:var(--leading-relaxed); white-space:pre; overflow-x:auto; margin-bottom:var(--space-8); color:var(--text-primary); }
    .faq-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(300px,1fr)); gap:var(--space-5); }
    .faq-card { background:var(--bg-surface); border:1px solid var(--bg-border); border-radius:var(--radius-xl); padding:var(--space-5); }
    .faq-card__q { font-size:var(--text-base); font-weight:var(--font-semibold); margin-bottom:var(--space-3); }
    .faq-card__a { font-size:var(--text-sm); color:var(--text-secondary); line-height:var(--leading-relaxed); }
    .cta-banner { background:var(--bg-raised); border:1px solid var(--bg-border); border-radius:var(--radius-xl); padding:var(--space-8); text-align:center; margin:var(--space-10) 0; }
    .cta-banner h2 { font-size:var(--text-xl); font-weight:var(--font-bold); margin-bottom:var(--space-3); }
    .cta-banner p { font-size:var(--text-base); color:var(--text-secondary); margin-bottom:var(--space-5); }
    .sidebar-links { margin-top:var(--space-3); padding-left:var(--space-4); }
    .sidebar-links li { font-size:var(--text-sm); margin-bottom:var(--space-2); }
    @media (min-width: 900px) {
      .content-layout { display:grid; grid-template-columns:1fr 280px; gap:var(--space-10); align-items:start; }
    }
    .sidebar { background:var(--bg-surface); border:1px solid var(--bg-border); border-radius:var(--radius-xl); padding:var(--space-5); position:sticky; top:80px; }
    .sidebar h3 { font-size:var(--text-sm); font-weight:var(--font-semibold); text-transform:uppercase; letter-spacing:0.08em; color:var(--text-muted); margin-bottom:var(--space-3); }
  </style>
</head>
<body>
  ...
  <main id="main-content">
    <div class="container" style="padding:var(--space-10) var(--space-4) var(--space-16)">
      <div id="breadcrumb-placeholder"></div>

      <div class="tool-hero" style="text-align:left; margin-bottom:var(--space-8)">
        <div class="tool-hero__badge">${page.category}</div>
        <h1 class="tool-hero__title">${page.h1}</h1>
        <p class="tool-hero__desc">${page.metaDesc}</p>
      </div>
      ...
      <div class="content-layout">
        ...
        <article class="prose">
          ...
          <h2>Code Example</h2>
          <div class="code-block">${page.codeExample}</div>

          <!-- CTA -->
          <div class="cta-banner">
            <h2>Try It Free — No Sign-Up Required</h2>
            <p>100% browser-based. Your data never leaves your machine.</p>
            <a href="${toolUrl}" class="btn btn--primary" style="padding:var(--space-3) var(--space-8);text-decoration:none;font-size:var(--text-base);font-weight:var(--font-semibold);">
              Open ${page.h1.split(':')[0]} →
            </a>
          </div>
          ...
          <section style="margin-top:var(--space-10)">
            ...
            <div class="faq-grid" style="margin-top:var(--space-5)">
              ${faqCardsHtml}
            </div>
          </section>
        </article>
        ...
        <aside class="sidebar">
          <h3>Open the Tool</h3>
          <a href="${toolUrl}" class="btn btn--primary" style="display:block;text-align:center;padding:var(--space-3);text-decoration:none;font-weight:var(--font-semibold);margin-bottom:var(--space-5);">
            Launch Tool →
          </a>

          ${relatedLinksHtml ? `<h3>Related Tools</h3><ul class="sidebar-links">${relatedLinksHtml}</ul>` : ''}
          ${otherArticlesHtml ? `<h3 style="margin-top:var(--space-4)">More Guides</h3><ul class="sidebar-links">${otherArticlesHtml}</ul>` : ''}

          <h3 style="margin-top:var(--space-4)">All Tools</h3>
          <ul class="sidebar-links">
            <li><a href="${BASE_URL}/tools/json-formatter.html" style="color:var(--accent);text-decoration:none;">JSON Formatter</a></li>
            <li><a href="${BASE_URL}/tools/json-validator.html" style="color:var(--accent);text-decoration:none;">JSON Validator</a></li>
            <li><a href="${BASE_URL}/tools/json-to-csv.html" style="color:var(--accent);text-decoration:none;">JSON to CSV</a></li>
            <li><a href="${BASE_URL}/tools/csv-to-json.html" style="color:var(--accent);text-decoration:none;">CSV to JSON</a></li>
            <li><a href="${BASE_URL}/tools/typescript-generator.html" style="color:var(--accent);text-decoration:none;">TypeScript Generator</a></li>
            <li><a href="${BASE_URL}/tools/json-to-yaml.html" style="color:var(--accent);text-decoration:none;">JSON to YAML</a></li>
            <li><a href="${BASE_URL}/tools/json-diff.html" style="color:var(--accent);text-decoration:none;">JSON Diff</a></li>
            <li><a href="${BASE_URL}/tools/json-to-sql.html" style="color:var(--accent);text-decoration:none;">JSON to SQL</a></li>
            <li><a href="${BASE_URL}/tools/jsonpath.html" style="color:var(--accent);text-decoration:none;">JSONPath Tester</a></li>
            <li><a href="${BASE_URL}/tools/json-schema-generator.html" style="color:var(--accent);text-decoration:none;">Schema Generator</a></li>
            <li><a href="${BASE_URL}/tools/json-to-prisma.html" style="color:var(--accent);text-decoration:none;">JSON to Prisma</a></li>
            <li><a href="${BASE_URL}/tools/json-to-drizzle.html" style="color:var(--accent);text-decoration:none;">JSON to Drizzle</a></li>
            <li><a href="${BASE_URL}/tools/json-to-graphql.html" style="color:var(--accent);text-decoration:none;">JSON to GraphQL</a></li>
            <li><a href="${BASE_URL}/tools/json-to-zod.html" style="color:var(--accent);text-decoration:none;">JSON to Zod</a></li>
          </ul>
        </aside>
      </div>
    </div>
  ...
  <script src="/assets/js/layout.js?v=2.8.0" defer></script>
</body>
</html>`;
}

// ── Hub Page ───────────────────────────────────────────────────
function renderHubHtml(pages) {
  // Group by toolSlug
  const byTool = {};
  for (const p of pages) {
    if (!byTool[p.toolSlug]) byTool[p.toolSlug] = [];
    byTool[p.toolSlug].push(p);
  }

  const toolGroupsHtml = Object.entries(byTool).map(([toolSlug, toolPages]) => {
    const linksHtml = toolPages.map(p =>
      `<li><a href="/kb/tools/${p.slug}.html" style="color:var(--accent);text-decoration:none;">${p.title.replace(' | JSON2X', '')}</a></li>`
    ).join('\n');
    const toolName = toolSlug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    return `
    <div class="faq-card">
      <h2 class="faq-card__q" style="font-size:var(--text-base)">${toolName}</h2>
      <ul style="padding-left:var(--space-4);margin-top:var(--space-3)">
        ${linksHtml}
      </ul>
      <a href="${BASE_URL}/tools/${toolSlug}.html" class="btn btn--ghost" style="margin-top:var(--space-4);display:inline-block;text-decoration:none;font-size:var(--text-xs)">Open Tool →</a>
    </div>`;
  }).join('\n');
...
<head>
  ...
  <title>JSON Tool Guides & SEO Hub — JSON Formatter, Validator, Converter Guides | JSON2X</title>
  <meta name="description" content="Comprehensive guides for every JSON2X tool. Learn how to use the JSON formatter, validator, minifier, CSV converter, TypeScript generator, Prisma/Drizzle schema generators, and more." />
  <meta name="keywords" content="json tool guides, json formatter guide, json validator guide, json to csv guide, json to typescript guide, json tools reference" />
  <meta name="robots" content="index, follow" />
  <meta name="theme-color" content="#0d1117" />
  <link rel="canonical" href="${BASE_URL}/kb/tools/index.html" />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="${BASE_URL}/kb/tools/index.html" />
  <meta property="og:title" content="JSON Tool Guides & Programmatic SEO Hub — JSON2X" />
  <meta property="og:description" content="In-depth guides covering every JSON2X tool. Format, validate, convert, diff, and generate schemas from JSON." />
  <meta property="og:site_name" content="JSON2X" />
  <meta property="og:image" content="${BASE_URL}/assets/og-image.png" />
  <script type="application/ld+json">
  { "@context":"https://schema.org","@type":"CollectionPage","name":"JSON Tool Guides","url":"${BASE_URL}/kb/tools/index.html" }
  </script>
  <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
  <link rel="preload" href="/assets/css/design-system.css" as="style" />
  <link rel="preload" href="/assets/css/components.css" as="style" />
  ...
  <main id="main-content">
    <div class="container" style="padding:var(--space-12) var(--space-4)">
      ...
      <div class="tool-hero" style="text-align:left;margin-bottom:var(--space-10)">
        <div class="tool-hero__badge">Tool Guides Hub</div>
        <h1 class="tool-hero__title">JSON Tool Guides &amp; Developer Reference</h1>
        <p class="tool-hero__desc">In-depth guides, tutorials, and keyword-targeted reference articles for every JSON2X tool. Browse by tool or use case.</p>
      </div>
      <div class="faq-grid" style="grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:var(--space-6)">
        ${toolGroupsHtml}
      </div>
    </div>
  </main>
...
// ── Build ──────────────────────────────────────────────────────
function buildToolPseo() {
  console.log('Building Per-Tool Programmatic SEO pages...');
  let count = 0;

  for (const page of TOOL_PAGES) {
    const html = renderToolPseoHtml(page, TOOL_PAGES);
    const outFile = path.join(OUT_DIR, `${page.slug}.html`);
    fs.writeFileSync(outFile, html, 'utf8');
    console.log(`   - Generated: kb/tools/${page.slug}.html`);
    count++;
  }

  const hubHtml = renderHubHtml(TOOL_PAGES);
  fs.writeFileSync(path.join(OUT_DIR, 'index.html'), hubHtml, 'utf8');

  console.log(`Successfully compiled ${count} tool pSEO pages + 1 hub in /kb/tools/`);
}
